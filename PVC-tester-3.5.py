...
Permet de lancer automatiquement une s�rie de solveurs sur une s�rie
de probl�mes
...
# Ces modules doivent �tre dans le PYTHONPATH; p.ex. dans le
# r�pertoire courant
...
def dist(city1, city2):
    """Compute distance between city 1 and 2."""
    x1, y1 = city1
    x2, y2 = city2
...
def validate(filename, length, path, duration, maxtime):
    ...
    try:
        cities = dict([(name, (int(x), int(y)))
                       for name, x, y in [l.split() for l in open(filename)]])
...
if __name__ == '__main__':
    ...
    # On met les diff�rentes fonctions ga_solve() dans un dictionnaire index�
    # par le nom du module correpsondant
    ...
    for (filename, maxtime) in tests:
        ...
        # Appel des solveurs proprement dits, v�rification et �criture des
        # r�sultats
        for m in modules:
            if verbose:
                print("## %s" % m)
